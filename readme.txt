================================================================================
         MANUAL BOOK - PETUNJUK PENGGUNA APLIKASI
...
Aplikasi Sistem Informasi Kursus Komputer adalah aplikasi desktop berbasis
Java Swing yang dirancang untuk mengelola operasional sebuah lembaga kursus
komputer. Aplikasi ini mencakup pengelolaan data peserta, instruktur, kursus,
...
Langkah 2 - Buat Database
  ...
  perintah SQL yang terdapat di file "database.sql" pada folder project ini.

  Cara melalui phpMyAdmin:
    a. Klik tab "Import"
    b. Pilih file "database.sql" dari folder project
    c. Klik tombol "Go" / "Eksekusi"
...
Langkah 3 - Verifikasi Konfigurasi Koneksi
  ...
  Jika MySQL Anda menggunakan password, ubah variabel "pass" di file
  src/uas/Koneksi.java sesuai password MySQL Anda.
...
Petunjuk:
  ...
  3. Klik tombol "Login"
  4. Jika berhasil, akan muncul pesan "Login Berhasil!" dan aplikasi
     akan mengarahkan ke halaman sesuai role:
...
Setelah login sebagai Admin, Anda akan masuk ke halaman Dashboard Admin.
Di sisi kiri terdapat sidebar navigasi dengan menu-menu berikut:
...
Fitur yang tersedia:
  a. Tambah Kursus
     ...
     - Isi form:
         * Nama Kursus : Nama kursus yang akan ditambahkan
                         (contoh: "Web Programming", "Data Science")
         * Harga       : Harga kursus dalam Rupiah (masukkan angka saja,
                         contoh: 500000)
     ...
     Validasi:
       ...
       - Jika berhasil, akan muncul pesan "Data Kursus Berhasil Disimpan!"
         dan otomatis kembali ke halaman daftar kursus
...
Setelah login sebagai Owner, Anda akan masuk ke halaman Dashboard Owner.
Panel Owner bersifat read-only (hanya melihat laporan, tidak bisa CRUD).
...
Aplikasi menyediakan 2 akun default yang sudah disiapkan di database:
  ...
  CATATAN: Disarankan untuk mengubah password default melalui phpMyAdmin
  atau query SQL untuk keamanan:
...
J: Pastikan MySQL Server sudah berjalan (cek XAMPP). Pastikan database
   "db_kursus" sudah dibuat dan konfigurasi koneksi di Koneksi.java
   sudah sesuai (host, port, username, password).

T: Muncul pesan "Username atau Password salah"?
J: Periksa kembali username dan password yang dimasukkan. Gunakan akun
   default: admin/admin123 (admin) atau owner/owner123 (owner).
   Password bersifat case-sensitive.

T: Data tidak muncul di tabel?
J: Pastikan tabel di database sudah berisi data. Jalankan file
   database.sql untuk mengisi data awal (seed data).

T: Tombol "Tambah Kursus" tidak berfungsi?
J: Pastikan Anda menggunakan versi terbaru dari file kursus.java.
   Tombol sudah dihubungkan ke panel Tambahkursus.

T: Field "Harga" menampilkan error saat simpan kursus?
J: Pastikan field Harga diisi dengan angka saja tanpa titik atau koma.
   Contoh yang benar: 500000
   Contoh yang salah: 500.000 atau Rp 500000

T: Bagaimana menambah akun pengguna baru?
J: Tambahkan langsung melalui phpMyAdmin atau MySQL CLI:
   INSERT INTO users (username, password, role)
   VALUES ('nama_user', 'password', 'admin');

T: Bagaimana cara mencetak laporan (pada panel Owner)?
J: Klik tombol "Cetak" pada halaman laporan. Sistem akan membuka
   dialog cetak (Print Dialog) dari sistem operasi Anda. Pastikan
   printer sudah terhubung, atau pilih "Print to PDF" untuk menyimpan
   sebagai file PDF.

T: Aplikasi berjalan lambat?
J: Pastikan MySQL Server berjalan dengan baik dan tidak ada proses
   berat lainnya. Restart aplikasi jika diperlukan.
...
================================================================================
              (c) 2026 - Sistem Informasi Kursus Komputer
